...
var executeCommand = function(command, expectedAction, callback) {
  try {
    ...
    // most likely connection to alarm has failed, try to
    // recover
    connectToAlarm();
    callback(err);
  };
...
// network issues seem to cause failures even with the
// handling already in place.
process.on('uncaughtException', () => setTimeout(connectToAlarm(), 5000));
